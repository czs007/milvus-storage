...
path: /tmp/RateLimited (temporary)"
    ...
    fn stream_errors_keep_classification_across_arrow_ffi() {
        ...
        assert!(
            message.contains("__LOON_FFI_ERRCODE__=109"),
            "{message}"
        );
        ...
        assert_eq!(code, 5); // EIO
        assert!(
            message.contains("__LOON_FFI_ERRCODE__=110"),
            "{message}"
        );
        ...
        assert_eq!(code, 5); // EIO
        assert!(
            message.contains("__LOON_FFI_ERRCODE__=104"),
            "{message}"
        );
        ...
        assert!(
            message.contains("__LOON_FFI_ERRCODE__=1002"),
            "{message}"
        );
        ...
        assert_eq!(code, 5); // EIO: semantic classification is in the marker.
        assert!(
            message.contains("__LOON_FFI_ERRCODE__=115"),
            "{message}"
        );
    ...
    fn scan_mode_errors_are_classified() {
        ...
        assert!(
            invalid
                .to_string()
                .contains("__LOON_FFI_ERRCODE__=115"),
            "{invalid}"
        );
    ...
    fn missing_pinned_snapshot_is_reported_as_not_found() {
        ...
        assert!(
            message.starts_with("__LOON_FFI_ERRCODE__=104; "),
