...
use crate::filesystem_c::{
    ...
    loon_errcode_arrow, loon_errcode_storage_access_denied, loon_errcode_storage_not_found,
    loon_errcode_storage_precondition_failed, loon_errcode_file_not_found,
    loon_errcode_not_support, loon_errcode_transient_throttling,
...
fn is_not_found_error_code(code: i32) -> bool {
    ...
    code == unsafe { loon_errcode_file_not_found } || code == unsafe { loon_errcode_storage_not_found }
}
...
fn into_opendal_error(
    ...
    } else if code == unsafe { loon_errcode_storage_access_denied }
    {
        opendal::ErrorKind::PermissionDenied
    } else if code == unsafe { loon_errcode_storage_precondition_failed } {
...
mod tests {
    ...
    fn generic_permission_denied_maps_to_permission_denied() {
        let mut result = LoonFFIResult {
            err_code: unsafe { crate::filesystem_c::loon_errcode_storage_access_denied },
