...
//! established in `filesystem_c.rs`. The marker is `__LOON_FFI_ERRCODE__=`,
//! shared with the C++ decoder in `runtime/bridge_error.cpp`.
//!
...
pub const BRIDGE_ERRCODE_MARKER: &str = "__LOON_FFI_ERRCODE__=";
const BRIDGE_ERRCODE_ESCAPED: &str = "__LOON_FFI_ESCAPED_ERRCODE__=";
...
// Mirrored for the vortex bridge's unit tests; production code classifies these
// two on the C++ side of the filesystem FFI.
#[allow(dead_code)]
...
// credential half.
/// `LanceError::Wrapped` boxes another error without exposing it through
/// `source()`, so a chain walk stops there; look through it explicitly.
fn unwrap_lance_error(e: &LanceError) -> &(dyn std::error::Error + 'static) {
    match e {
        LanceError::Wrapped { error, .. } => match error.downcast_ref::<LanceError>() {
            Some(inner) => unwrap_lance_error(inner),
            None => error.as_ref(),
        },
        _ => e,
    }
...
pub fn classify_lance_error(e: &LanceError) -> Option<i32> {
    match e {
        LanceError::Wrapped { error, .. } => match error.downcast_ref::<LanceError>() {
            Some(inner) => classify_lance_error(inner),
            None => classify_error_source_chain(Some(error.as_ref())),
        },
        ...
        LanceError::IO { source, .. } => {
            ...
            match source.downcast_ref::<object_store::Error>() {
                ...
                Some(object_store::Error::Generic { source, .. }) => source
                    ...
                    .or_else(|| classify_error_source_chain(Some(source.as_ref()))),
                // Anything else (a bare io::Error, a custom store failure): the
                // C++ filesystem verdict may still sit further down the chain.
                _ => classify_error_source_chain(Some(source.as_ref())),
            }
        }
...
/// The C++ filesystem's own verdict, when it sits anywhere in the chain. The
/// filesystem-backed Lance/Iceberg adapters (#636/#653) wrap `LoonFFIError`
/// inside object_store / opendal errors; its `err_code` is the producer's
/// classification and its fields are the diagnostic without any wrapper text.
fn loon_ffi_error_in_chain<'a>(
    mut source: Option<&'a (dyn std::error::Error + 'static)>,
) -> Option<&'a crate::filesystem_c::LoonFFIError> {
    while let Some(error) = source {
        if let Some(error) = error.downcast_ref::<crate::filesystem_c::LoonFFIError>() {
            return Some(error);
        }
        source = error.source();
    }
...
pub(crate) fn into_arrow_io_error(error: BridgeError) -> arrow::error::ArrowError {
    let message = match error.code {
        Some(code) => format!("{BRIDGE_ERRCODE_MARKER}{code}; {}", error.msg),
        None => error.msg,
    };
    arrow::error::ArrowError::IoError(message.clone(), std::io::Error::other(message))
}
...
impl From<LanceError> for BridgeError {
    fn from(e: LanceError) -> Self {
        let code = classify_lance_error(&e);
        // A failure that originated in the C++ filesystem already carries the
        // producer's context and message; lance's "Wrapped error: LanceError(IO):
        // Generic filesystem_c error: ..." chain adds nothing a consumer can act
        // on and would only carry the (escaped) inner marker along.
        if let Some(ffi) = loon_ffi_error_in_chain(Some(unwrap_lance_error(&e))) {
            return BridgeError::new_io(
                code.or(Some(ffi.err_code)),
                format!("{}: {}", ffi.context, ffi.message),
            );
        }
        let io_transport = bridge_io_transport_in_source_chain(Some(unwrap_lance_error(&e)));
        BridgeError::with_transport(code, e.to_string(), io_transport)
    }
}
...
mod tests {
    ...
    struct ErrorReader {
        error: Option<arrow::error::ArrowError>,
    ...
    impl arrow::record_batch::RecordBatchReader for ErrorReader {
    ...
    fn ffi_stream_error(error: BridgeError) -> (i32, String) {
        ...
        let mut stream = arrow::ffi_stream::FFI_ArrowArrayStream::new(Box::new(reader));
    ...
    fn classified_stream_error_has_one_arrow_io_envelope() {
        for error in [
            ...
            assert!(message.starts_with("Io error: __LOON_FFI_ERRCODE__="));
    ...
    fn marker_code_round_trips_the_cxx_decoder_contract() {
        ...
        assert_eq!(
            marker_code_in("vortex: __LOON_FFI_ERRCODE__; odd"),
            None
        );
        assert_eq!(
            marker_code_in(&format!(
