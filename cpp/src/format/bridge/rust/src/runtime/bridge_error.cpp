...
constexpr std::string_view kBridgeErrCodeMarker = kFFIErrorCodeMarker;
...
ParsedBridgeError ParseBridgeError(std::string_view error) {
  // Only a marker at byte zero, or immediately after Arrow's own C-stream
  // "Io error: " wrapper, is a frame. Marker text anywhere else can be part of a
  // caller-provided URI/path inside an ordinary diagnostic and is not framing;
  // the Rust producers additionally escape marker literals in caller text.
  size_t marker_pos = 0;
  bool io_transport = false;
  if (error.starts_with(kBridgeErrCodeMarker)) {
    ...
    marker_pos = kArrowIoPrefix.size();
    io_transport = true;
  } else {
  ...
  // marker-like text is not enough: malformed text must stay an ordinary
  // diagnostic instead of entering the classification channel.
  ...
  // "Io error: " is Arrow's own framing, not part of the diagnostic.
  const auto strip_from = io_transport ? kArrowIoPrefix.size() : 0;
  return {StripBridgeMarker(error.substr(strip_from), marker_pos - strip_from, code_end - strip_from), ffi_err_code,
          true, io_transport};
}
