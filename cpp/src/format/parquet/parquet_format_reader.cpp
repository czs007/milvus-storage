...
// On this synchronous path a supplied footer size is authoritative: a failed
// pre-read is returned rather than retried through Arrow's normal footer path.
// (The asynchronous open in read_footer_async treats the footer size as an
// optimization hint and falls back to Parquet's native footer read instead.)
static arrow::Result<std::shared_ptr<arrow::Buffer>> read_footer_buffer(
    const std::shared_ptr<arrow::io::RandomAccessFile>& file, uint64_t file_size, uint64_t footer_size) {
