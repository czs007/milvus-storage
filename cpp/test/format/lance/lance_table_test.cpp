...
TEST_F(LanceBasicTest, MaterializedReadsTranslateDeferredErrors) {
  ...
  // EACCES from the C++ filesystem crosses the FFI as StorageAccessDenied (105);
  // there is no separate errno-only permission code any more.
  auto permission_detail = ExtendStatusDetail::UnwrapStatus(permission_result.status());
  ASSERT_NE(permission_detail, nullptr) << permission_result.status().ToString();
  EXPECT_EQ(permission_detail->code(), ExtendStatusCode::StorageAccessDenied);
