...
TEST_F(IcebergBridgeTest, PlanFilesNonexistentLocalMetadataReturnsNotFound) {
  ...
  // Bridges report a missing object through the taxonomy channel (StorageNotFound)
  // rather than the filesystem's errno channel; both land on the same segcore code.
  auto detail = ExtendStatusDetail::UnwrapStatus(result.status());
  ASSERT_NE(detail, nullptr) << result.status().ToString();
  EXPECT_EQ(detail->code(), ExtendStatusCode::StorageNotFound) << result.status().ToString();
}
...
TEST_F(IcebergBridgeTest, PlanFilesPreservesFilesystemErrorDetails) {
  ...
  for (auto code : {ExtendStatusCode::StorageAccessDenied, ExtendStatusCode::StorageTransientThrottling}) {
